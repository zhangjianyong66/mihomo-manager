...
- `Enter`：确认或进入。
- `Esc`：返回。
- 节点列表中 `t`：空闲时测试光标所在节点；单测进行中把光标节点加入串行队列；批量测速中切换为该节点单测。
- 节点列表中 `a`：立即测试当前代理组全部可测速节点；已有单测队列或批量测速会被取消并重新开始批测。
...
- 节点管理：进入节点列表时显示 mihomo 运行时的最新历史测速结果，不自动测速；连续单测按入队顺序串行执行，单节点或批量测速只更新结果，按 `Enter` 才切换节点。
- 订阅管理：保存、查看和更新订阅。
- 白名单：添加、删除和查看直连域名。
