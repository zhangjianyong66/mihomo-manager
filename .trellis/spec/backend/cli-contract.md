...
错误：single 模式再次按 `t` 仍调用会取消当前 context 的启动入口；测试只断言返回了非空 command。

```go
return m.startNodeTest(singleRequest(nodeID), nodeTestSingle)
```

正确：当前节点和队列先去重，其他节点只追加到队尾并即时反馈；只有队首完成后才为下一节点返回新 command。

```go
if m.nodeTestMode == nodeTestSingle {
    if nodeID == m.nodeTestCurrent || m.nodeTestQueued(nodeID) {
        return m, nil
    }
