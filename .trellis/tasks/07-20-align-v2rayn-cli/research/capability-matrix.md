...
|---|---|---|---|---|---|
| 稳定非交互接口 | GUI 操作与内部服务层 | 已有命令工厂、`mm/v1` JSON、错误/退出码和脱敏契约；业务子命令未接入 | 部分 | 2.0 Alpha | 核心能力均提供 CLI、JSON、稳定退出码 |
...
| Core 生命周期 | 启停、重启、切换、日志 | 已有启停、重启、重载、测试和日志 | 已有 | 2.0 Alpha | 等价迁移并纳入 daemon 单写者模型 |
...
| 节点和代理组 | 列表、筛选、选择、分组 | 已有代理组、节点列表和切换 | 部分 | 2.0 Alpha/Beta | Alpha 等价迁移；Beta 使用来源与稳定 ID 管理 |
| 延迟与速度测试 | 多种延迟/速度测试 | mihomo API 延迟测试、并发限制和最快切换 | 部分 | 2.0 Alpha/2.1 | Alpha 等价迁移；2.1 增加测试策略和真实吞吐测试 |
...
| 日志 | core 与客户端日志 | 已有尾部读取、跟随、正则过滤 | 部分 | 2.0 Alpha/2.1 | Alpha 等价迁移；2.1 统一脱敏、轮转和查询 |
