...
func NewDaemonService(paths config.ManagerPaths) *DaemonService {
	return &DaemonService{
		...
		Controller: newDaemonController(paths),
...
func protocolCategory(code string) ErrorCategory {
	switch code {
	case "REQUEST_ID_CONFLICT", "OPERATION_CONFLICT", "CONFLICT", "CONFIG_CHANGED", "PORT_CONFLICT", "PROFILE_MODE_UNSUPPORTED", "UNSUPPORTED_PROFILE", "PROXY_AUTH_UNSUPPORTED", "RULESET_NOT_READY":
		return ErrorCategoryConflict
	case "INVALID_REQUEST", "INVALID_ROUTING_MODE", "REQUEST_ID_REQUIRED", "PLATFORM_UNSUPPORTED", "RULESET_PROXY_AUTH_UNSUPPORTED":
